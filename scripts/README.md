...
`scripts/` 提供本仓库稳定的人类操作入口。它们是薄 wrapper，真正的配置合同仍由 `src/lib/catalog/project-schema.ts` 和 `src/lib/catalog/projects.ts` 执行。
...
| `verify.sh` | build/catalog/content 验证。 | README 或 `docs/` 漂移检查。 |
| `catalog.sh` | `catalog/projects/<id>/` 的 list、validate、new。 | GitHub API 抓取、taxonomy 自动修改、schema 之外的字段生成。 |
...
`catalog` 和 `content` 当前都通过 `npm run build` 触发可执行校验。未来如果构建变慢，可以新增更窄的 catalog-only 校验，但仍应复用 schema/loader，不在 shell 里重写合同。
